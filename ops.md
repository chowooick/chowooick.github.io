...
`--no-xattrs`가 경고를 없앤다. 그 폴더가 실행 중인 컨테이너에 바인드 마운트되어 있으면 `rm -rf`로 새로 만든
디렉터리는 **다른 inode**라 컨테이너가 옛 디렉터리를 계속 볼 수 있다 — 동기화 뒤 `docker restart <컨테이너>`로
마운트를 다시 잡는다(misa 유저는 `docker` 그룹이라 sudo 없이 된다).
